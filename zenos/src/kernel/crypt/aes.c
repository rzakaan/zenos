...
static void multiply(state_t* state);
static void key_expansion(uint8_t* round_key, const uint8_t* key);
static void chipher(state_t* state, const uint8_t* round_key);
static void inv_chipher(state_t* state, const uint8_t* round_key);
static void aes_encrypt(const struct AES_CTX* ctx, uint8_t* buf);
static void aes_decrypt(const struct AES_CTX* ctx, uint8_t* buf);

/*
 * Substitutes values in the state matrix with values in an S-Box
 */
static void sub_bytes(state_t* state) {
    uint8_t i, j;
    for (i = 0; i < 4; ++i) {
...
static void chipher(state_t* state, const uint8_t* round_key) {
    ...
    // add first round key to the state before starting the rounds.
...
static void aes_decryptaes_encrypt(const struct AES_CTX* ctx, uint8_t* buf) {
    chipher((state_t*)buf, ctx->round_key);
}

static void aes_decrypt(const struct AES_CTX* ctx, uint8_t* buf) {
    //dechipher((state_t*)buf, ctx->round_key);
}